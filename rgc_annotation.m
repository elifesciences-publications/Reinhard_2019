function rgc_annotation(files,datapath)
...
for file = 1:length(files)
    ...
    if exist(strcat('/media/areca_raid/VNet/Dataset/TBP/',FileTif), 'file') ~= 2
        ...
        im=zeros(nImage,mImage,NumberImages,'uint8');%im=zeros(nImage,mImage,NumberImages,'uint8');
...
for file = 1:length(files)
    ...
    cnnname = strcat(fname, 'chAT_STD_rotate.tif');
        InfoImage=imfinfo(fullfile(datapath,FileTif));  %this is dst image to project to
%     InfoImage=imfinfo(strcat('/media/areca_raid/VNet/ImagesHere/',FileTif));  %this is dst image to project to
    ...
    %%%%%Same thing for OFF%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %%%%%%%%%%%%%%%%%call Detect ON, OFF 
        DetectONOFFSurface3(imcnnOutON, imcnnOutOFF,cnnname,datapath);
...
% %        
         ...
         DetectONOFFSurface4(imcnnOutONOFF, cnnname,datapath);
     end
